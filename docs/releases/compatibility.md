...
| --- | --- | --- |
| Flutter `tugboat` | 0.10.2 | Android `capture-runtime` `0.1.0`; Apple `TugboatCaptureRuntime` `0.1.1`; Device Farm launch inputs owned by the SDK; HTTP error `network_call` mapping; `hidden` + `paused` coalesce into one `app_backgrounded`. |
