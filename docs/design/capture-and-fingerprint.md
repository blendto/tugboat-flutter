# Tugboat Capture & Fingerprint — Design

Status: Draft for review · Owner: Tugboat · Scope: `tugboat` (Dart SDK) + `tugboat-cli` (ingestion)

## 1. Problem & goals

Tugboat builds a **content graph** of an app by exploring it (today: a CLI + LLM driving
...
3. **No reliance on developer tags.** `TugboatTag` is an optional hardening tier, not a
...
- The SDK captures rich anchors (`labelHashes`/`actionableRoles`/`iconHashes`, target
  anchors with bounds), but the consumer (`tugboat-cli`) **ignores almost all of it**.
...
[release build + dormant SDK]
        ...
        ▼
   CAPTURE  (tugboat) ── fingerprints, screenshots, logs, network, perf, analytics
        │
   TRANSPORT (pluggable sink) ── WebSocketSink (dev) | FileSink (pull) | HttpBatchSink (scale)
        ▼
   INGESTION (tugboat-cli / backend) ── builds content graph; joins prod events by (apkSha256, fingerprintSchemaVersion, fingerprint)
```
...
`RepaintBoundary` + `NotificationListener` (`lib/src/tugboat.dart`, `initState` /
...
- not inside a `TugboatSensitive` subtree.
...
### 4.5 `TugboatTag` — new public API

`TugboatTag` is a public marker in `markers.dart` and the package exports:
...
A `TugboatTag` (or a `ValueKey<String>`) on/above an element emits a separate
...
high-value controls in `TugboatTag` or give them a stable `ValueKey<String>`. Structural
...
- **high** — an explicit `TugboatTag`/`ValueKey<String>` alias.
...
`TugboatTag` is transparent to canonical paths and route fallback. Adding one therefore does
...
`scrollStartEventId`. The context graph service preserves these fields in `/v1/enrichment/map-event` summaries so
production sessions can identify scroll confusion without replaying raw payloads. The collector
consumer stores the full mapping at `payload.contextEnrichment` at ingest; event-aware labels
include `Scroll did not move on …` for dead swipes. See `tugboat-collector/docs/enrichment.md`.
...
- **Self-instrumentation:** the SDK measures its own per-interaction time and frame impact
  (via `SchedulerBinding.addTimingsCallback`), emits a `tugboat.overhead` signal, and
...
- **Analytics tap-in**, recommended in order:
  ...
  2. Bridge API `Tugboat.recordAnalytics(name, props)` + thin adapters for
...
- **Activation config** (runtime, since released APKs can't take `--dart-define`):
  - Android: `adb am start -n <pkg>/.MainActivity --es tugboat_session <id> --es tugboat_sink <endpoint> --es tugboat_profile exploration`. A small native hook (bundled in the SDK) reads intent extras and calls `Tugboat.activate(...)`.
  - Alternative: deep link (`app://tugboat/capture?...`) handled by the same hook.
...
- Screenshots: `TugboatSensitive` always masks. Exploration otherwise remains
...
1. **Fingerprint v2 + join (narrow)** in `tugboat` + `tugboat-cli`:
...
- **Route key for nested navigators / tabs / dialogs**: confirm the §4.2 chain covers
  `TugboatSubView`, modal routes, and tabbed shells.
