...
- `userId` — current runtime user on `session_start`, `session_identify`,
  `user_changed`, and `traits_updated`. Omitted on `session_end`. After start,
  `null` is an explicit identity-clear on `user_changed` / `session_identify`
  and the current (possibly anonymous) identity on `traits_updated`;
- `traits` — full traits snapshot when the host has set a bag (`session_start`,
  `session_identify`, `traits_updated`); the collector stores the bag as-is
  (no server-side partial merge). `user_changed` does not send traits;
