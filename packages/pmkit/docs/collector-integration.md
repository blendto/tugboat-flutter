...
`pmkit_cli` under `frames/`. ADB step screenshots remain the primary gesture-level evidence.
...
during `pmkit_cli` recordings. ADB screenshots remain the visual evidence per step. Tracked in
`pmkit_cli/docs/recording-data-quality-plan.md` (Phase 1 + SDK cross-repo item).
...
that *are* captured are streamed over the socket and persisted under `frames/` by `pmkit_cli`.
