...
void main() {
  ...
  test('tap_settled result prefers signature change over stale frame ids', () {
    ...
    final result = controller.debugComputeTapSettleResult(
      beforeState: const PmkitStateAnchor(signature: 'sig-before'),
      afterState: const PmkitStateAnchor(signature: 'sig-after'),
      beforeFrame: 'frame-1',
      afterFrame: 'frame-1',
    );
    expect(result, PmkitInteractionResult.changed);
    controller.dispose();
  });
  ...
  test('tap_settled result uses tap-down baseline signatures', () {
    ...
    final result = controller.debugComputeTapSettleResult(
      beforeState: const PmkitStateAnchor(signature: 'home-sig'),
      afterState: const PmkitStateAnchor(signature: 'route-sig'),
      beforeFrame: 'frame-1',
      afterFrame: 'frame-1',
    );
    expect(result, PmkitInteractionResult.changed);
