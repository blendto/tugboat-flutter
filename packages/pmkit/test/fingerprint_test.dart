...
import 'package:pmkit/src/anchors.dart'
    show AnchorResolver, pmkitFingerprintSchemaVersion, pmkitLabelHash;
...
void main() {
  testWidgets('fingerprints are deterministic for the same widget tree', (
    ...
    Future<PmkitStateAnchor> capture() async {
    ...
    expect(first.schemaVersion, pmkitFingerprintSchemaVersion);
  });
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      Future<PmkitTargetAnchor?> tapRow(int index) async {
        await tester.pumpWidget(
          MaterialApp(
            home: RepaintBoundary(
              key: rootKey,
  ...
  testWidgets('static discriminators distinguish list items', (tester) async {
    ...
    Future<PmkitTargetAnchor?> tapPlan(String label) async {
      await tester.pumpWidget(
        MaterialApp(
          home: RepaintBoundary(
            key: rootKey,
  ...
  testWidgets('PmkitTag adds an alias without changing structural identity', (
    tester,
  ) async {
    final rootKey = GlobalKey();

    Future<(PmkitTargetAnchor, PmkitStateAnchor)> capture({
      ...
      await tester.pumpWidget(
        MaterialApp(
          home: RepaintBoundary(
            key: rootKey,
            child: Scaffold(
              body: tagged ? PmkitTag('checkout-cta', child: button) : button,
            ),
          ),
        ),
      );
  ...
  testWidgets('target fingerprint excludes schema version metadata', (
    ...
    expect(
      anchor.fingerprint,
      pmkitLabelHash('path=${anchor.canonicalPath}|routeKey=/home'),
    );
    expect(anchor.schemaVersion, pmkitFingerprintSchemaVersion);
  });
  ...
  testWidgets('state signature excludes schema version metadata', (
    ...
    expect(state.signature, pmkitLabelHash('actionablePaths=|routeKey=/home'));
    expect(state.schemaVersion, pmkitFingerprintSchemaVersion);
  });
